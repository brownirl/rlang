...
def _build_model(feature_network, head_network):
...
class CraftworldInnerDQNPreset(Preset):
    """
    Deep Q-Network (DQN) for Hierarchical Craftworld 
    Inner Agent Implementation with common Feature Network.
    ...
    def __init__(self, env, name, device, **hyperparameters):
        ...
        if hyperparameters['feature_constructor'] is None:
            self.model = hyperparameters['model_constructor'](env).to(device)
        else:
            self.model = _build_model(hyperparameters['feature_network'], hyperparameters['model_constructor'](env)).to(device)
...
craft_inner_dqn = partial(PresetBuilder, constructor=CraftworldInnerDQNPreset)
