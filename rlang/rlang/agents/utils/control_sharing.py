...
if __name__=="__main__":
    ...
    print(
        f"Prob0: {beta * torch.exp(a[0]) + (1 - beta) * torch.exp(b[0])}, Prob1: {beta * torch.exp(a[1]) + (1 - beta) * torch.exp(b[1])}")
    assert (beta * torch.exp(a) + (1 - beta) * torch.exp(b) == torch.exp(
        dist.log_prob(torch.arange(a.size()[-1])))).all()
