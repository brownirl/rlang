...
class SimpleRLAgent(simple_rl_agent.Agent):
    ...
    def act(self, state, reward):
        return self._agent.act({'observation': state_builder(state.features()), 'reward': reward, 'done': state.is_terminal()})
    
    def policy(self, state):
        return self._agent.eval({'observation': state_builder(state.features()), 'reward': 0, 'done': state.is_terminal()})    
