...
def conditional_reward_function(if_condition, if_reward, elif_conditions=None, elif_rewards=None, else_reward=None,
                                *args, **kwargs):
    if if_condition(*args, **kwargs) == True:
        return if_reward(*args, **kwargs) if if_reward else None
    else:
        for i in range(len(elif_conditions)):
            if elif_conditions[i](*args, **kwargs) == True:
                return elif_rewards[i](*args, **kwargs) if elif_rewards[i] else None
        else_reward(*args, **kwargs) if else_reward else None
...
def conditional_transition_function(if_condition, if_transition, elif_conditions=None, elif_transitions=None,
    ...
    if if_condition(*args, **kwargs) == True:
        return if_transition(*args, **kwargs) if if_transition else None
    else:
        for i in range(len(elif_conditions)):
            if elif_conditions[i](*args, **kwargs) == True:
                return elif_transitions[i](*args, **kwargs) if elif_transitions[i] else None
        if else_transition is not None:
            return else_transition(*args, **kwargs) if else_transition else None
        else:
            return None
...
def conditional_prediction_function(if_condition, if_prediction, elif_conditions=None, elif_predictions=None,
    ...
    if if_condition(*args, **kwargs) == True:
        return if_prediction(*args, **kwargs) if if_prediction else None
    else:
        for i in range(len(elif_conditions)):
            if elif_conditions[i](*args, **kwargs) == True:
                return elif_predictions[i](*args, **kwargs) if elif_predictions[i] else None
        return else_prediction(*args, **kwargs) if else_prediction else None
