...
class ReinforceMLPAgent(ReinforceAgent):
    ...
    def __init__(self, actions, state_dim=1, name="", gamma=0.95, alpha=0.01, N=5, hidden_size=16, n_hidden_layers=1):
        ...
        layers = [torch.nn.Linear(state_dim, hidden_size), torch.nn.ReLU()] + [
            torch.nn.Linear(hidden_size, hidden_size), torch.nn.ReLU()] * n_hidden_layers + [
                     torch.nn.Linear(hidden_size, len(self.actions)), ]
    ...
    def _loss(self):
        ...
        for t in self.trajectories:
            ...
            logprob = torch.gather(torch.log(torch.nn.functional.softmax(self.policy_params(states), dim=-1)), -1,
                                   actions).squeeze()
