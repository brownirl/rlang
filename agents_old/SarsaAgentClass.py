'''
    SARSA Agent for Discrete MDPs
    author: Roma Patel (romapatel@brown.edu)
'''
...
class SarsaAgent(Agent):
    ...
    def __init__(self, actions, name="SARSA", alpha=0.1, gamma=0.99,
        ...
        Args:
            ...
            custom_q_init (defaultdict{state, defaultdict{action, float}}):
             a dictionary of dictionaries storing the initial q-values.
             Can be used for potential shaping (Wiewiora, 2003)
            default_q (float): the default value to initialize every entry
            in the q-table with [by default, set to 0.0]
        '''
    ...
    def get_pred_q_value(self, state):
        """
        For SARSA
        :param state:
        :return:
        """
        qval = self._compute_max_qval_action_pair(state)
        a2 = qval[1]
        # a2 = random_action(a2, eps=0.5/t) # epsilon-greedy

        return self.get_q_value(state, a2)
        # return self._compute_max_qval_action_pair(state)[0]
